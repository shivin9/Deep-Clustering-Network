...
if __name__ == '__main__':
    ...
    parser.add_argument('--input-dim', type=float, default=89, 
    ...
    X_train = list(zip(X_train.astype(np.float32), y_train.to_numpy().astype(np.float32).ravel()))
    X_test  = list(zip(X_test.astype(np.float32), y_test.to_numpy().astype(np.float32).ravel()))
